...
#define PLAT_LOCAL_PSTATE_MASK		((1 << PLAT_LOCAL_PSTATE_WIDTH) - 1)
	...
	//#define PLAT_ARM_TRUSTED_DRAM_BASE	UL(0x1f000000)
...
#define ARM_SHARED_RAM_BASE			(BL31_BASE + 0x00020000 - ARM_SHARED_RAM_SIZE) // UL(0xfffCE000) // Hila: add calc ARM_TRUSTED_SRAM_BASE
...
#define ARM_BL_RAM_BASE			(ARM_SHARED_RAM_BASE +	\
					 ...
					 //PLAT_ARM_TRUSTED_SRAM_SIZE is taken from platform_def.h 0x20000 because only half is 
					// secured in this specific implementation !
...
#define MAP_DEVICE0	MAP_REGION_FLAT(PLAT_REG_BASE,			\
					PLAT_REG_SIZE,			\
...
#define MAP_DEVICE1	MAP_REGION_FLAT(DEVICE1_BASE,			\
	...
	// The following section is used for HW validation of TrustZone on NPCM845x				
#if 0
...
#define PLAT_NPCM_TM_HOLD_BASE		(PLAT_NPCM_TM_ENTRYPOINT + \
					 PLAT_NPCM_TM_ENTRYPOINT_SIZE)
#define PLAT_NPCM_TM_HOLD_ENTRY_SIZE	ULL(8)
#define PLAT_NPCM_TM_HOLD_SIZE		(PLAT_NPCM_TM_HOLD_ENTRY_SIZE * \
					 PLATFORM_CORE_COUNT)

#define PLAT_NPCM_TRUSTED_NOTIFICATION_BASE	(PLAT_NPCM_TM_ENTRYPOINT_SIZE + \
					 PLAT_NPCM_TM_HOLD_SIZE)
					 
#define PLAT_NPCM_TRUSTED_NOTIFICATION_ENTRY_SIZE	ULL(8)

#define PLAT_NPCM_TRUSTED_NOTIFICATION_SIZE		(PLAT_NPCM_TRUSTED_NOTIFICATION_ENTRY_SIZE * \
					 PLATFORM_CORE_COUNT)
					 
		
	
#define PLAT_NPCM_TRUSTED_MAILBOX_SIZE	(PLAT_NPCM_TM_ENTRYPOINT_SIZE + \
					 PLAT_NPCM_TM_HOLD_SIZE + PLAT_NPCM_TRUSTED_NOTIFICATION_ENTRY_SIZE)
